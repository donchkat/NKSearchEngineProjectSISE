...
class MultiFileWriter:
    ...
    def upload_to_gcp(self,dir_name):
        '''
            The function saves the posting files into the right bucket in google storage.
        '''
        file_name = self._f.name
        blob = self.bucket.blob(f"postings_gcp/{dir_name}/{file_name}")
        blob.upload_from_filename(file_name)#might be problematic
...
class MultiFileReader:
    ...
    def __init__(self):
        ...
        # Connecting to google storage bucket. 
        self.client = storage.Client()
        self.bucket = self.client.bucket('katya_niv_title_index')
        
    def read(self, locs, n_bytes):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = self.bucket.get_blob(f'anchor_text_index/postings_gcp/at_index_files/{f_name}')
...
class InvertedIndex:  
    ...
    def read_index(base_dir, name):
        ...
        blob = bucket.blob(f"{base_dir}/{name}")
...
#             return pickle.load(f)
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, bucket_name,dir_name):
